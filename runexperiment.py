...
import os
import random
import model_change_test
#run this file to begin the experiment
from net import NetSimple,ghostnet,NetConcat
# 全局参数定义
...
def draw(acc,auc,f1,loss,f_acc,f_auc,f_f1,REALEPACH,name):
    x = np.arange(REALEPACH+1)
    ...
    plt.savefig('./result/NetConcat_Simple_proce_fg_'+NAME+'.png')
    plt.savefig(fname='./result/'+NAME+'.svg', format='svg')
    plt.show()
...
def train(net,name):
    ...
    for epoch in range(EPOCHMAX):
        ...
        auc_avg += roc_auc_score(label, np.array(prob_auc))
        acc_avg += 100 * correct / total
        #print(label,prob_f1)
        f1_avg += f1_score(label, np.array(prob_f1))
        print("AUC:{:.4f}".format(roc_auc_score(label, np.array(prob_auc))))
        print("F1-Score:{:.4f}".format(f1_score(label, np.array(prob_f1))))
        print('Accuracy of the network on the 7:3 test images: %f %%' % (100 * correct / total))
        auc.append(roc_auc_score(label, np.array(prob_auc)))
        f1.append(f1_score(label, np.array(prob_f1)))
        ...
        if ((epoch + 1) % 10 == 0):  # 每10论计算一下 是否相较上次个相差在0.1  是的话停止
            ...
            if (abs(roc_auc_score(label, np.array(prob_auc)) - auc_avg) < 0.001 and abs(
                    f1_score(label, np.array(prob_f1)) - f1_avg) < 0.001 and abs(
                    running_loss / len - loss_avg) < 0.001):
                f_acc = correct / total
                f_auc = roc_auc_score(label, np.array(prob_auc))
                f_f1 = f1_score(label, np.array(prob_f1))
                break
            else:
                acc_avg = 0
                auc_avg = 0
        ...
        REALEPOCH += 1
        running_loss = 0.0
    draw(acc, auc, f1, loss_all, f_acc, f_auc, f_f1,REALEPOCH,name)
    torch.save(net.state_dict(), 'net/' + name + '.pth')
...
if __name__ =='__main__':
    ...
    # 测试网络列表
    netList = [model_change_test.Net(),NetConcat()]
    nameList = ['NetSimple']
    NAME = 'data7_3_orgin_rui7'
